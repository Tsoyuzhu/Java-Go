...
public class GoGame {
    ...
    private List<EnumPositionState> positionStates;

    // Keep a copy of the last state to check whether the KO rule has been broken
    private List<EnumPositionState> previousState;
    ...
    // Determine if game as ended
    private EnumGameStatus gameStatus;

    public GoGame() {
        // Init board with empty states
        positionStates = new ArrayList<EnumPositionState>(SIZE*SIZE);
        previousState = new ArrayList<EnumPositionState>(SIZE*SIZE);
        ...
        gameStatus = EnumGameStatus.CREATED;
    ...
    public void handleMoveRequest(GameMove gameMove) throws Exception {
        ...
        if (gameMove.getMoveType().equals(EnumMoveType.PASS)) {
            if (this.previousPlayerPassed) {
                ...
                this.gameStatus = EnumGameStatus.COMPLETED;
            } else {
        ...
        if (history.isEmpty()) {
            gameStatus = EnumGameStatus.ONGOING;
        }
